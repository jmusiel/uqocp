...
@registry.register_model("residual_ensemble_equiformer_v2")
class residual_EquiformerV2(EquiformerV2_OC20):
    ...
    def __init__(
        ...
        self.residual_energy_blocks = nn.ModuleList()
        for i in range(10):
            ...
            self.residual_energy_blocks.append(block)
    ...
    def forward(self, data):
        ...
        ###############################################################
        ensemble_residuals = []
        for i, block in enumerate(self.residual_energy_blocks):
            residual_node_energy = block(x)
            ...
            print(f"Res Energy ({i}): {residual_energy}")
            residual_energy = torch.abs(energy - residual_energy)
            ensemble_residuals.append(residual_energy)

        ensemble_residuals = torch.stack(ensemble_residuals, dim=1)
        residual_energy = torch.mean(ensemble_residuals, dim=1)
