...
def get_system_errors(tid):
    ...
    if all_exist:
        ...
        for arr, model in zip(forces_array, checkpoints[1:]):
            temp_dict[model.split("/")[-1]] = np.linalg.norm(arr - mean_force, axis=2).tolist()
        temp_dict["en_error"] = mean_error.tolist()
        temp_dict["en_stdev"] = stdev.tolist()
    return temp_dict
